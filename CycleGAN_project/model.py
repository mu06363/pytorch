...
class CycleGAN(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm='inorm'):
        ...
        self.enc1 = CBR2d(in_channels, 1 * nker, kernel_size=7, stride=1, padding=(7-1)//2, norm=norm, relu=0.0)

        self.enc2 = CBR2d(1 * nker, 2 * nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu=0.0)

        self.enc3 = CBR2d(2 * nker, 4 * nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu=0.0)

        res = []
        for i in range(9):
            res += [ResBlock(4 * nker, 4 * nker, kernel_size=3, stride=1, padding=(3-1)//2, norm=norm, relu=0.0)]
        self.res = nn.Sequential(*res)

        self.dec1 = DECBR2d(4 * nker, 2 * nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu=0.0)

        self.dec2 = DECBR2d(2 * nker, 1 * nker, kernel_size=3, stride=2, padding=(3-1)//2, norm=norm, relu=0.0)

        self.dec3 = CBR2d(1 * nker, out_channels, kernel_size=7, stride=1, padding=(7-1)//2, norm=None, relu=None)
...
class Pix2Pix(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, norm='bnorm'):
        ...
        self.enc1 = CBR2d(in_channels, 1 * nker, kernel_size=4, padding=1, stride=2, norm=None, relu=0.2)

        self.enc2 = CBR2d(1 * nker, 2 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.enc3 = CBR2d(2 * nker, 4 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.enc4 = CBR2d(4 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.enc5 = CBR2d(8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)
        
        self.enc6 = CBR2d(8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.enc7 = CBR2d(8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.enc8 = CBR2d(8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.2)

        self.dec1 = DECBR2d(8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)
        self.drop1 = nn.Dropout2d(0.5)

        self.dec2 = DECBR2d(2 * 8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)
        self.drop2 = nn.Dropout2d(0.5)

        self.dec3 = DECBR2d(2 * 8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)
        self.drop3 = nn.Dropout2d(0.5)

        self.dec4 = DECBR2d(2 * 8 * nker, 8 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)

        self.dec5 = DECBR2d(2 * 8 * nker, 4 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)

        self.dec6 = DECBR2d(2 * 4 * nker, 2 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)

        self.dec7 = DECBR2d(2 * 2 * nker, 1 * nker, kernel_size=4, padding=1, stride=2, norm=norm, relu=0.0)

        self.dec8 = DECBR2d(2 * 1 * nker, out_channels, kernel_size=4, padding=1, stride=2, norm=None, relu=None)
...
class UNet(nn.Module):
    def __init__(self, in_channels, out_channels, nker, norm='bnorm', learning_type='plain'):
        ...
        self.dec4_2 = CBR2d(in_channels=16*nker, out_channels=8*nker, norm=norm)
        ...
        self.dec3_2 = CBR2d(in_channels=8*nker, out_channels=4*nker, norm=norm)
        ...
        self.dec2_2 = CBR2d(in_channels=4*nker, out_channels=2*nker, norm=norm)
        ...
        self.dec1_2 = CBR2d(in_channels=2*nker, out_channels=1*nker, norm=norm)
...
class Hourglass(nn.Module):
    def __init__(self, in_channels, out_channels, nker, norm='bnorm', learning_type='plain'):
        ...
        # Expansive path
        self.dec5_1 = CBR2d(in_channels=8*nker, out_channels=8*nker, norm=norm)
...
class SRResNet(nn.Module):
    def __init__(self, in_channels, out_channels, nker=64, learning_rate='plain', norm='bnorm', nblk=16):
        ...
        ps1 = []
        ps1 += [nn.Conv2d(in_channels=nker, out_channels=nker * 4, kernel_size=3, stride=1, padding=1)]
        ps1 += [PixelShuffle(ry=2, rx=2)]
        ps1 += [nn.ReLU()]
        self.ps1 = nn.Sequential(*ps1)

        ps2 = []
        ps2 += [nn.Conv2d(in_channels=nker, out_channels=nker * 4, kernel_size=3, stride=1, padding=1)]
