    ...
    if mode == 'train':
        ...
        for epoch in range(st_epoch + 1, num_epoch + 1):
            ...
            for batch, data in enumerate(loader_train, 1):
                ...
                real = torch.cat([label, input], dim=1)
                ...
                print("TRAIN: EPOCH %04d / %04d | BATCH %04d / %04d | GEN L1 %.4f | GEN GAN %.4f | DISC REAL: %.4f | DISC FAKE: %.4f" %
                    (epoch, num_epoch, batch, num_batch_train, np.mean(loss_G_l1_train), np.mean(loss_G_gan_train), np.mean(loss_D_real_train), np.mean(loss_D_fake_train)))
                ...
                if batch & 20 == 0:
                    ...
                    plt.imsave(os.path.join(result_dir_train, 'png', '%04d_label.png' % id), label[0].squeeze())
            ...
            with torch.no_grad():
                ...
                for batch, data in enumerate(loader_val, 1):
                    ...
                    print("VALID: EPOCH %04d / %04d | BATCH %04d / %04d | GEN L1 %.4f | GEN GAN %.4f | DISC REAL: %.4f | DISC FAKE: %.4f" %
                        (epoch, num_epoch, batch, num_batch_val, np.mean(loss_G_l1_val), np.mean(loss_G_gan_val), np.mean(loss_D_real_val), np.mean(loss_D_fake_val)))
                    ...
                    if batch & 10 == 0:
                        ...
                        plt.imsave(os.path.join(result_dir_val, 'png', '%04d_label.png' % id), label[0].squeeze())
    ...
    if mode == 'test':
        ...
        with torch.no_grad():
            ...
            for batch, data in enumerate(loader_test, 1):
                ...
                print("VALID: BATCH %04d / %04d | GEN L1 %.4f" %
                ...
                if batch & 10 == 0:
                    ...
                    for j in range(lael.shape[0]):
                        ...
                        plt.imsave(os.path.join(result_dir_test, 'png', '%04d_input.png' % id), input_.squeeze())
                        plt.imsave(os.path.join(result_dir_test, 'png', '%04d_label.png' % id), label_.squeeze())
                        plt.imsave(os.path.join(result_dir_test, 'png', '%04d_output.png' % id), output_.squeeze())
